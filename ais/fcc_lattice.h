...
public:
	...
	vector< vector<int> > nn, nnn; //nearest neighbour, next nearest neighbour
	map<int, int> mp; //get the index of position in vector<int> pos
	
	vector<vector<set<int>>> nns_lists;

	vector<vector<pair<int,int>>> nn_lists, bond_lists;
	// first : index of site
	// second : index in Jr
	typedef vector<pair<int,int>>::iterator pair_iter;
	...
	FCC(const int L_, const bool pbc_) : L(L_), pbc(pbc_)
	{
		...
		int LEN = L * 2; 
		// get all lattice locations
		for(int x=0;x<LEN;x+=2){
			for(int y=0;y<LEN;y+=2){
				for(int y=0;y<LEN;y+=2){
					const int p0 = LEN*LEN*x + LEN*y + z;
					const int p1 = LEN*LEN*(x+1) + LEN*(y+1) + z;
					const int p2 = LEN*LEN*(x+1) + LEN*y + (z+1);
					const int p3 = LEN*LEN*x + LEN*(y+1) + (z+1);
		...
		std::set<size_t> dist_list;
		std::map<size_t,int> dist_index;


		for(auto p : pos){
			const int x = (p/LEN/LEN)%LEN;
			const int y = (p/LEN)%LEN;
			const int z = (p)%LEN;
			
			const int dx = std::min(x,LEN-x);
			const int dy = std::min(y,LEN-y);
			const int dz = std::min(z,LEN-z);

			const size_t r2 = dx*dx+dy*dy+dz*dz;
		...
		nns_lists.resize(pos.size());
		for(int i=0;i<pos.size();i++){
			nns_lists[i].resize(dist_index.size());
		}

		for(auto d : pos){ // loop over all lattice sites and find there "distance" from the origin
			const int x = (d/LEN/LEN)%LEN;
			const int y = (d/LEN)%LEN;
			const int z = (d)%LEN;
			...
			if(r2 > 0){ 
				int i=0;
				const int nn = dist_index[r2];

				for(auto p : pos){
					const int px = (p/LEN/LEN)%LEN;
					const int py = (p/LEN)%LEN;
					const int pz = (p)%LEN;

					const int qx = (px+x)%LEN;
					const int qy = (py+y)%LEN;
					const int qz = (pz+z)%LEN;

					const int q = LEN*LEN*qx + LEN*qy + qz;

					if(mp.count(q) > 0){
						auto qnn = make_pair(j,nn);
						nns_lists[i][nn].insert(mp[q]);
						nn_lists[i].push_back(qnn);
						if(p<q){bond_lists[i].push_back(qnn);}
					}
